...
import { readdir, stat as stat2, access as access3, mkdir as mkdir2 } from "node:fs/promises";
import { join as join5, extname as extname2, basename as basename3, relative } from "node:path";

// src/main/services/archival/archival.service.ts
import { spawn as spawn4 } from "node:child_process";
import { randomUUID } from "node:crypto";
import { access as access2, mkdir, rename, rm, stat, unlink as unlink2 } from "node:fs/promises";
import { basename as basename2, dirname as dirname3, extname, join as join4, parse as parse2 } from "node:path";
...
import { accessSync, constants, existsSync as existsSync2, mkdirSync, readFileSync } from "node:fs";
import { basename, dirname as dirname2, join as join2, parse } from "node:path";
var WhisperService = class {
  ...
  async transcribeWithWhisperCpp(request) {
    ...
    const outputDir = request.outputDir ?? dirname2(request.audioPath);
    const baseName = parse(request.audioPath).name;
    const outputPrefix = join2(outputDir, baseName);
  ...
  async transcribeWithFasterWhisper(request, command) {
    ...
    const outputDir = request.outputDir ?? dirname2(request.audioPath);
    const baseName = parse(request.audioPath).name;
    const outputPath = join2(outputDir, `${baseName}.txt`);
    mkdirSync(outputDir, { recursive: true });
  ...
  async runProcess(command, args, signal, onLog) {
    return new Promise((resolve, reject) => {
      ...
      const child = spawn2(command, args, { stdio: "pipe" });
      let stderr = "";
      let settled = false;
...
function buildTwoPassArgs(inputPath, outputPath, config, sourceInfo, passLogDir, cpuCapabilities) {
  const { basename: basename4, join: join6 } = __require("node:path");
  const inputName = basename4(inputPath, __require("node:path").extname(inputPath));
  const passLogFile = join6(passLogDir, `${inputName}-pass`);
  if (config.codec === "h265") {
...
function getEncoderList(ffmpegPath) {
  return new Promise((resolve, reject) => {
    const proc = spawn3(ffmpegPath, ["-encoders", "-hide_banner"]);
    let stdout = "";
...
var ArchivalStatePersistence = class {
  ...
  constructor() {
    ...
    this.statePath = join3(app.getPath("userData"), STATE_FILE_NAME);
  }
  /**
...
var ArchivalService = class {
  ...
  async resumeFromRecovery(state) {
    ...
    this.isPaused = false;
    try {
      await access2(this.activeJob.config.outputDir);
    } catch {
      this.activeJob = null;
  ...
  async getBatchInfo(inputPaths, outputDir) {
    ...
    for (const inputPath of inputPaths) {
      try {
        ...
        const inputName = basename2(inputPath, extname(inputPath));
        for (const ext of containerExtensions) {
          const outputPath = join4(outputDir, `${inputName}.${ext}`);
          try {
            await access2(outputPath);
            existingCount++;
            break;
          } catch {
          }
  ...
  async processItem(item) {
    ...
    try {
      ...
      await mkdir(dirname3(item.outputPath), { recursive: true });
      ...
      if (outputLarger && this.activeJob.config.deleteOutputIfLarger) {
        ...
        this.logger.info("Skipped - output larger than input", {
          input: basename2(item.inputPath),
      ...
      this.logger.info(`Completed archival encoding${warningMsg}`, {
        input: basename2(item.inputPath),
        output: basename2(item.outputPath),
  ...
  async encodeTwoPass(item, sourceInfo, cpuCapabilities) {
    ...
    const passLogDir = join4(dirname3(item.outputPath), ".pass-logs");
    ...
    try {
      ...
      this.logger.info("Starting two-pass encoding - Pass 1", { input: basename2(item.inputPath) });
      this.emitEvent({
        batchId,
        itemId: item.id,
        kind: "item_progress",
      ...
      this.logger.info("Starting two-pass encoding - Pass 2", { input: basename2(item.inputPath) });
  ...
  runFFmpegPass(item, sourceInfo, args, passNumber, batchId) {
    return new Promise((resolve, reject) => {
      ...
      const proc = spawn4(ffmpegPath, fullArgs, { stdio: ["ignore", "pipe", "pipe"] });
  ...
  encodeSinglePass(item, sourceInfo, cpuCapabilities) {
    return new Promise((resolve, reject) => {
      ...
      const proc = spawn4(ffmpegPath, args, { stdio: ["ignore", "pipe", "pipe"] });
      this.activeProcess = proc;
  ...
  buildOutputPath(inputPath, outputDir, config, relativePath) {
    ...
    if (config.preserveStructure && relativePath) {
      const relativeDir = dirname3(relativePath);
      if (relativeDir && relativeDir !== ".") {
        outputPath = join4(outputDir, relativeDir, `${inputName}.${extension}`);
      } else {
        outputPath = join4(outputDir, `${inputName}.${extension}`);
      }
    } else {
      outputPath = join4(outputDir, `${inputName}.${extension}`);
    }
  ...
  async extractThumbnail(videoPath, sourceInfo, thumbnailTimestamp) {
    ...
    const videoDir = dirname3(videoPath);
    const videoName = basename2(videoPath, extname(videoPath));
    const thumbnailPath = join4(videoDir, `${videoName}.jpg`);
    return new Promise((resolve, reject) => {
      const args = [
        "-ss",
        String(timestampSec),
      ...
      const abortHandler = () => {
        proc.kill();
        void unlink2(thumbnailPath).catch(() => {
        });
      };
      this.abortController?.signal.addEventListener("abort", abortHandler, { once: true });
      proc.on("close", async (code) => {
        ...
        if (this.abortController?.signal.aborted) {
          await unlink2(thumbnailPath).catch(() => {
          });
  ...
  async extractCaptions(videoPath, language) {
    ...
    const ffmpegPath = resolveBundledBinary("ffmpeg");
    const videoDir = dirname3(videoPath);
    const videoName = parse2(videoPath).name;
    const tempAudioName = `${videoName}_whisper_temp`;
    const audioPath = join4(videoDir, `${tempAudioName}.wav`);
    const vttOutputPath = join4(videoDir, `${tempAudioName}.vtt`);
    const txtOutputPath = join4(videoDir, `${tempAudioName}.txt`);
    const finalVttPath = join4(videoDir, `${videoName}.vtt`);
    ...
    try {
      ...
      if (useLmStudio) {
        ...
        try {
          await unlink2(finalVttPath).catch(() => {
          });
      ...
      await unlink2(audioPath).catch(() => {
      });
      await unlink2(txtOutputPath).catch(() => {
      });
    }
  }
  /**
...
${colors.cyan}\u2192${colors.reset} Starting: analyzing video...`);
      ...
      case "item_progress":
        if (event.progress !== void 0) {
          ...
          const batchEta = event.batchEtaSeconds !== void 0 ? formatEta(event.batchEtaSeconds) : "--";
          clearLine();
          process.stdout.write(
            `  ${progressBar(progress)} ${progress.toString().padStart(3)}%  ${colors.dim}Speed:${colors.reset} ${speed}  ${colors.dim}ETA:${colors.reset} ${eta}  ${colors.dim}Batch:${colors.reset} ${batchProgress}% (${batchEta})`
          );
        }
        break;
      case "item_complete":
        ...
        if (event.status === "completed") {
          ...
          console.log(
            `  ${colors.green}\u2713${colors.reset} Completed  ${colors.dim}Size:${colors.reset} ${size}  ${colors.dim}Ratio:${colors.reset} ${ratio}`
          );
...
var archiveCommand = new Command("archive").description("Batch encode videos using AV1 or H.265").argument("<input>", "Input file or directory").argument("<output>", "Output directory").option("-c, --codec <codec>", "Codec to use: av1 or h265", "av1").option("-p, --preset <preset>", "Quality preset: archive, max-compression, or fast", "archive").option("--crf <number>", "Custom CRF value (overrides preset)").option("--container <format>", "Container format: mkv, mp4, or webm", "mkv").option("--overwrite", "Overwrite existing output files", false).option("--preserve-structure", "Preserve folder structure from input", false).option("--delete-if-larger", "Delete output if larger than input", true).option("--two-pass", "Enable two-pass encoding for better quality", false).option("--threads <number>", "Limit encoder threads (0 = unlimited)", "0").option("-r, --recursive", "Process directories recursively", true).option("-v, --verbose", "Show detailed progress", false).action(async (input, output, options) => {
  try {
    console.log(`
...
`);
    ...
    const inputStat = await stat2(input);
    let inputPaths = [];
    let relativePaths = [];
    let folderRoot;
